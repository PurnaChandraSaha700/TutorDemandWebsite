...
session_start();
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (isset($_POST['searchBtn'])){
                ...
                $_SESSION['searchEmailinUpdateStudentByAdmin'] = $searchEmail;
                ...
                $sql = "SELECT * FROM studentTable WHERE studentEmail = '$searchEmail'";
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        $displayProfilePicture = $row["studentProfilePicture"];
        ...
        elseif (isset($_POST['submit'])){
            ...
            $updateEmail = $_SESSION['searchEmailinUpdateStudentByAdmin'];
            $conn =new mysqli('localhost','root','','tutorquestdatabase');
            if($conn->connect_error){
                die('connection failed : '.$conn->connect_error);
            }
            ...
            $studentCountPost = $_POST['studentCountPost'];
            $tutorCount = $_POST['tutorCount'];
            $isVarifiedStudent = $_POST['isVarifiedStudent'];
            ...
            $sql = "UPDATE studenttable SET studentFirstName=?, studentLastName=?, studentDOB=?, studentGender=?, studentPhone=?, studentPermanentAddress=?, studentCurrentAddress=?, studentCity=?, studentZipCode=?, studentPassword=?, studentBio=?,studentCountPost=?, tutorCount=?, isVarifiedStudent=?  WHERE studentEmail = '$updateEmail'";

            // Prepare and bind the statement
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssssssssssiii", $studentFirstName, $studentLastName, $studentDOB, $studentGender, $studentPhone, $studentPermanentAddress, $studentCurrentAddress, $studentCity, $studentZipCode, $studentPassword, $studentBio,$studentCountPost,$tutorCount,$isVarifiedStudent);
            ...
            <div class="col-lg-8">
                <div class="profile-picture">
                    <?php if ($displayProfilePicture): ?>
                        <img src="data:image/jpeg;base64,<?php echo base64_encode($displayProfilePicture); ?>" alt="Profile Picture" style="max-width: 180px; max-height 100px">
                    <?php else: ?>
                ...
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="additional-section">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="Password" name="Password" value="<?php echo $studentPassword; ?>">
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label for="studentCountPost" class="form-label">Number Of Posts</label>
                                <input type="text" class="form-control" id="studentCountPost" name="studentCountPost" value="<?php echo $studentCountPost; ?>">
                            </div>
                            <div class="col-md-6">
                                <label for="tutorCount" class="form-label">Tutor Count:</label>
                                <input type="text" class="form-control" id="tutorCount" name="tutorCount" value="<?php echo $tutorCount; ?>">
                            </div>
                        </div>

                        <!-- is varified student  -->
                        <div class="mb-3">
                            <label for="isVarifiedStudent" class="form-label">Is Verified Student?</label>
                            <select class="form-select" id="isVarifiedStudent" name="isVarifiedStudent">
                                <option value="1"<?php if ($isVarifiedStudent == 1) echo ' selected'; ?>>Yes</option>
                                <option value="0"<?php if ($isVarifiedStudent == 0) echo ' selected'; ?>>No</option>
                            </select>
                        </div>
